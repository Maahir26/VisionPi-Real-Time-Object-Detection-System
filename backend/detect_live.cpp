/**
 * @file detect_live.cpp
 * @author Juan
 * @brief This file handles the live video detection
 * @version 0.1
 * @date 2024-04-01
 * 
...
/**
 * @brief main functional that deals with real time detection
 * 
 * @return int 
 */
int main(int, char**) {
    ...
    while (cap.isOpened()) {
        ...
        imshow("image", image); //i dont think i need this because it displays or opens the window
        out.write(image);
        int k = waitKey(10);
        if (k == 113){ ///this means if the button q is pressed, it will quit
            break;
        }
    }
